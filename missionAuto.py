...
class MissionAuto(QtWidgets.QWidget):
    ...
    def init_ui(self):
        ...
        self.title_label = QtWidgets.QLabel("请选择你要执行的任务脚本（Esc键可中断任务）")
    ...
    def mission_picker(self):
        ...
        # log脚本重载：脚本列表
        self.print_and_log(f"脚本载入：{self.script_list}")
    ...
    def handle_single_action(self, record: pd.Series, time_interval: int):
        ...
        match record["操作"]:
            case "左键单击":
                ...
                self.buttun_trail("left", record, time_interval)
            case "左键双击":
            ...
            case "右键单击":
                ...
                self.buttun_trail("right", record, time_interval)
            case "左键长按":
    ...
    def buttun_trail(self, button: str, record: pd.Series, time_interval: int):
        while True:
            ...
            for i in range(30):
                ...
                # 尝试根据坐标点击鼠标
                if button_position_1 is None:
                    pag.click(button=button, x=record["状态"][0], y=record["状态"][1])
        ...
        pag.click(button=button, x=button_position_1[0], y=button_position_1[1])
